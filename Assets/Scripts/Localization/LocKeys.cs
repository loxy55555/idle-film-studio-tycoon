/// <summary>Localization key constants — no user-facing strings in code.</summary>
public static class LocKeys
{
    public const string DeptLevelFormat     = "dept.level_format";
    public const string DeptUpgrade         = "dept.upgrade";
    public const string DeptMax             = "dept.max";
    public const string DeptLocked          = "dept.locked";
    public const string DeptAvailableCity   = "dept.available_city";
    public const string DeptBonusSummary    = "dept.bonus_summary";
    public const string DeptCategoryQuality = "dept.category.quality";
    public const string DeptCategorySpeed   = "dept.category.speed";
    public const string DeptCategoryReduction = "dept.category.reduction";

    public const string DeptNameEditor         = "dept.name.editor";
    public const string DeptNameDirector       = "dept.name.director";
    public const string DeptNameActors         = "dept.name.actors";
    public const string DeptNameSound          = "dept.name.sound";
    public const string DeptNameCinematography = "dept.name.cinematography";
    public const string DeptNameMakeup         = "dept.name.makeup";
    public const string DeptNameCostume        = "dept.name.costume";
    public const string DeptNameArt            = "dept.name.art";
    public const string DeptNameLighting       = "dept.name.lighting";
    public const string DeptNameGrip           = "dept.name.grip";
    public const string DeptNameProducer       = "dept.name.producer";

    public const string ProdTabTitle           = "prod.tab_title";
    public const string ProdNewProduction      = "prod.new_production";
    public const string ProdActiveSection      = "prod.active_section";
    public const string ProdNoActive           = "prod.no_active";
    public const string ProdStartMovie         = "prod.start_movie";
    public const string ProdStatusProducing    = "prod.status_producing";
    public const string ProdSelect             = "prod.select";
    public const string ProdCatalogComplete    = "prod.catalog_complete";
    public const string ProdNoOffer            = "prod.no_offer";
    public const string ProdEmptySlot          = "prod.empty_slot";
    public const string ProdDurationFormat     = "prod.duration_format";
    public const string ProdTimeRemainingSeconds = "prod.time_remaining_seconds";
    public const string ProdTimeRemainingMinutes = "prod.time_remaining_minutes";
    public const string ProdHistoryRewards     = "prod.history_rewards";

    public const string ProdBudgetTitle        = "prod.budget.title";
    public const string ProdBudgetSubtitle     = "prod.budget.subtitle";
    public const string ProdBudgetCheap        = "prod.budget.cheap";
    public const string ProdBudgetStandard     = "prod.budget.standard";
    public const string ProdBudgetPremium      = "prod.budget.premium";
    public const string ProdBudgetCancel       = "prod.budget.cancel";
    public const string ProdBudgetStatBlock    = "prod.budget.stat_block";

    public const string ProdOfferMoney         = "prod.offer.money";
    public const string ProdOfferRep           = "prod.offer.rep";

    public const string RefreshChoosePayment   = "refresh.choose_payment";
    public const string RefreshWatchAd         = "refresh.watch_ad";
    public const string RefreshSpendDiamonds   = "refresh.spend_diamonds";

    public const string ContractRefresh        = "contract.refresh";
    public const string ContractSelect         = "contract.select";
    public const string ContractChoosePrompt   = "contract.choose_prompt";
    public const string ContractActiveObjective = "contract.active_objective";
    public const string ContractObjProduceMovies = "contract.obj.produce_movies";
    public const string ContractObjProduceGenre  = "contract.obj.produce_genre";
    public const string ContractObjReachRep      = "contract.obj.reach_rep";
    public const string ContractObjReachQuality  = "contract.obj.reach_quality";
    public const string ContractObjEarnMoney     = "contract.obj.earn_money";
    public const string ContractObjSpendUpgrades = "contract.obj.spend_upgrades";
    public const string ContractObjReachLevel    = "contract.obj.reach_level";
    public const string ContractObjUnderTime     = "contract.obj.under_time";

    public const string ProdRarityCommon       = "prod.rarity.common";
    public const string ProdRarityRare         = "prod.rarity.rare";
    public const string ProdRarityEpic         = "prod.rarity.epic";
    public const string ProdRarityLegendary    = "prod.rarity.legendary";

    public const string GenreAction            = "genre.action";
    public const string GenreDrama             = "genre.drama";
    public const string GenreHorror            = "genre.horror";
    public const string GenreComedy            = "genre.comedy";
    public const string GenreRomance           = "genre.romance";
    public const string GenreSciFi             = "genre.scifi";
    public const string GenreFantasy           = "genre.fantasy";
    public const string GenreThriller          = "genre.thriller";
    public const string GenreAnimation         = "genre.animation";
    public const string GenreDocumentary       = "genre.documentary";

    public const string ProdPremiereHook       = "prod.hook.premiere";
    public const string ProdRewardsHook        = "prod.hook.rewards";
    public const string ProdDiscoveryHook      = "prod.hook.discovery";

    public const string FtueWelcomeBody        = "ftue.welcome_body";
    public const string FtueProductionTitle    = "ftue.production_title";
    public const string FtueProductionBody     = "ftue.production_body";
    public const string FtueStudioTitle        = "ftue.studio_title";
    public const string FtueStudioBody         = "ftue.studio_body";
    public const string FtueFirstMovieTitle    = "ftue.first_movie_title";
    public const string FtueCompleteBody       = "ftue.complete_body";
    public const string FtueMoneyEarned        = "ftue.money_earned";
    public const string FtueRepEarned          = "ftue.rep_earned";
    public const string FtueMovieDiscovered    = "ftue.movie_discovered";
    public const string FtueContinue           = "ftue.continue";
    public const string FtueSkip               = "ftue.skip";

    public const string PremiereCompleted      = "premiere.completed";
    public const string PremiereRewardsTitle   = "premiere.rewards_title";
    public const string PremiereMoneyReward    = "premiere.money_reward";
    public const string PremiereRepReward      = "premiere.rep_reward";
    public const string PremiereDiscoveryTitle = "premiere.discovery_title";
    public const string PremiereContinue       = "premiere.continue";

    public const string CollectionTitle          = "collection.title";
    public const string CollectionGlobalProgress = "collection.global_progress";
    public const string CollectionPercent        = "collection.percent";
    public const string CollectionGenreProgress  = "collection.genre_progress";
    public const string CollectionUnknownTitle   = "collection.unknown_title";
    public const string CollectionLocked         = "collection.locked";
    public const string CollectionDiscovered     = "collection.discovered";
    public const string CollectionLegendaryLock  = "collection.legendary_lock";
    public const string CollectionSelectGenre    = "collection.select_genre";
    public const string CollectionAllMovies      = "collection.all_movies";
    public const string CollectionBack           = "collection.back";

    // ─── Instalaciones (replaces Ciudad in UX) ────────────────────────────────
    public const string InstLevel           = "inst.level";
    public const string InstAvailable       = "inst.available";
    public const string InstProgressScreen  = "inst.progress_screen";
    public const string InstUpgrade         = "inst.upgrade";
    public const string InstNextLevel       = "inst.next_level";
    public const string InstCurrentLevel    = "inst.current_level";
    public const string InstMultiplier      = "inst.multiplier";

    // ─── Bonifications panel (Studio screen) ─────────────────────────────────
    public const string BonifSpeed          = "bonif.speed";
    public const string BonifQuality        = "bonif.quality";
    public const string BonifBoxOffice      = "bonif.box_office";
    public const string BonifRep            = "bonif.rep";
    public const string BonifXP             = "bonif.xp";
    public const string BonifCosts          = "bonif.costs";
    public const string BonifTitle          = "bonif.title";

    // ─── Settings overlay ──────────────────────────────────────────────────────
    public const string SettingsTitle       = "settings.title";
    public const string SettingsLanguage    = "settings.language";
    public const string SettingsGooglePlay  = "settings.google_play";
    public const string SettingsAccount     = "settings.account";
    public const string SettingsSupport     = "settings.support";
    public const string SettingsCredits     = "settings.credits";
    public const string SettingsPrivacy     = "settings.privacy";
    public const string SettingsRestore     = "settings.restore";
    public const string SettingsClose       = "settings.close";

    // ─── Tienda (Shop) placeholder ────────────────────────────────────────────
    public const string TiendaTitle         = "tienda.title";
    public const string TiendaComingSoon    = "tienda.coming_soon";
}

public static class DepartmentLocKeys
{
    public static string NameKey(DepartmentType type) => type switch
    {
        DepartmentType.Editor         => LocKeys.DeptNameEditor,
        DepartmentType.Director       => LocKeys.DeptNameDirector,
        DepartmentType.Actors         => LocKeys.DeptNameActors,
        DepartmentType.Sound          => LocKeys.DeptNameSound,
        DepartmentType.Cinematography => LocKeys.DeptNameCinematography,
        DepartmentType.Makeup         => LocKeys.DeptNameMakeup,
        DepartmentType.Costume        => LocKeys.DeptNameCostume,
        DepartmentType.Art            => LocKeys.DeptNameArt,
        DepartmentType.Lighting       => LocKeys.DeptNameLighting,
        DepartmentType.Grip           => LocKeys.DeptNameGrip,
        DepartmentType.Producer       => LocKeys.DeptNameProducer,
        _                             => LocKeys.DeptNameEditor,
    };
}
